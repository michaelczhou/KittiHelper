...
std::vector<float> read_lidar_data(const std::string lidar_data_path)
{
    ...
    const size_t num_elements = lidar_data_file.tellg() / sizeof(float);   //tellg()返回当前关联的 streambuf 对象的输入位置指示器。
...
int main(int argc, char** argv)
{
    ...
    n.getParam("dataset_folder", dataset_folder);  //数据集路径
    n.getParam("sequence_number", sequence_number);  //kitti序列号
    std::cout << "Reading sequence " << sequence_number << " from " << dataset_folder << '\n';
    bool to_bag;
    n.getParam("to_bag", to_bag);   //是否保存bag文件标志位
    if (to_bag)
        n.getParam("output_bag_file", output_bag_file);   //若保存的话保存到此路径
    int publish_delay;
    n.getParam("publish_delay", publish_delay);   //和频率相关
    ...
    odomGT.header.frame_id = "/camera_init";  //里程计真值为啥定义两个坐标系???
    odomGT.child_frame_id = "/ground_truth";

    ros::Publisher pubPathGT = n.advertise<nav_msgs::Path>("/path_gt", 5);
    nav_msgs::Path pathGT;
    pathGT.header.frame_id = "/camera_init";    //真值就是一个坐标系
    ...
    R_transform << 0, 0, 1, -1, 0, 0, 0, -1, 0;   //初始旋转为什么是这样？？
    ...
    {
        float timestamp = stof(line);  //转译浮点值
        ...
        for (std::size_t i = 0; i < 3; ++i)
        {
            for (std::size_t j = 0; j < 4; ++j)
            {
                ...
                gt_pose(i, j) = stof(s);  //stof 转译浮点值
            }
        }

        Eigen::Quaterniond q_w_i(gt_pose.topLeftCorner<3, 3>()); //取出旋转矩阵
        Eigen::Quaterniond q = q_transform * q_w_i;     //转了下坐标系,具体是从？？？转到？？？
        q.normalize();   //归一化的作用:只有单位化的四元数才用来描述旋转,因为转换过程中可能存在漂移，所以要进行归一化。而且四元数的归一化要比旋转矩阵归一化简单的多
        Eigen::Vector3d t = q_transform * gt_pose.topRightCorner<3, 1>();  //取平移的三个量
